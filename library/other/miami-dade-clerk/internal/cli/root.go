...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "miami-dade-clerk-pp-cli",
		Short: `Pull the full lien chain on any Miami-Dade property in 30 seconds — every deed, mortgage, satisfaction, lis pendens, and federal tax lien ever recorded.`,
		Long: `Pull the full lien chain on any Miami-Dade property in 30 seconds — every deed, mortgage, satisfaction, lis pendens, and federal tax lien ever recorded.

Highlights (not in the official API docs):
  • lien-chain   See every recording ever filed against a property in one chronological timeline — deeds, mortgages, satisfactions, lis pendens, federal tax liens, assignments — without manually deed-walking owner-by…
  • surviving-liens   Tells you which liens on a property will survive a foreclosure or tax-deed sale, with totals in cents — Federal Tax Liens always survive tax deed; junior mortgages get wiped on senior foreclosure; HO…
  ...
  • case-arc   Walks a single court case across all its recorded documents in chronological order: lis pendens → final judgment → certificate of title → satisfaction, with a status classifier (open / judgment-enter…
  • enrich   For a list of folios, compute lien-chain + surviving-liens + chain-of-title for each, and emit a single JSON file with totals_cents, surviving_lien_count, oldest_deed_date, last_deed_date, current_ow…
...
Health check: run 'miami-dade-clerk-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("miami-dade-clerk-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'miami-dade-clerk-pp-cli profile list')")
	...
	rootCmd.AddCommand(newEnvironmentCmd(flags))
	...
	rootCmd.AddCommand(newDocTypesPromotedCmd(flags))
	rootCmd.AddCommand(newSearchNamePromotedCmd(flags))
	rootCmd.AddCommand(newSearchPropertyPromotedCmd(flags))
	rootCmd.AddCommand(newSearchRecordingPromotedCmd(flags))
	rootCmd.AddCommand(newSearchLegalPromotedCmd(flags))
	rootCmd.AddCommand(newSearchResultsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Transcendence features — hand-coded analytics on top of the
	// synced recordings table. Each reads from the local SQLite store
	// (run `sync recordings` first) and emits structured JSON.
	rootCmd.AddCommand(newLienChainCmd(flags))
	rootCmd.AddCommand(newSurvivingLiensCmd(flags))
	rootCmd.AddCommand(newChainOfTitleCmd(flags))
	rootCmd.AddCommand(newOwnerPortfolioCmd(flags))
	rootCmd.AddCommand(newCaseArcCmd(flags))
	rootCmd.AddCommand(newEnrichCmd(flags))
	rootCmd.AddCommand(newFTLScanCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("miami-dade-clerk-pp-cli %s\n", version)
		},
	}
}
